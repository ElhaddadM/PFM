...
function Service_A() {
    ...
    const handleEmail = (e) => { (/(.+)@(.+){2,}\.(.+){3}/).test(e.target.value) && (e.target.value).length > 0 ? setEmail('mb-3 form-control is-valid ') && setCounta(counta + 1) : setEmail("mb-3 form-control is-invalid ") }
    ...
    const handleValider = () => {  
        ...
        axios.post("http://rdv.local/api/email/token", ok)
            .then((response) => {
    ...
    const handleClickValider = () => {
        ...
        axios.post("http://rdv.local/api/rdv", RDV)
            .then((response) => {
                console.log(response);
                setFooterM({ btnsA: 'd-none', btnsB: 'd-none', prise: 'd-none', confirmer: 'd-block' })
                axios.post("http://rdv.local/api/email/confirm", status)
                    .then((response) => {
                        console.log("ok");
    ...
    return (
        <div className=" border my-4 bg-white shadow-lg p-3 mb-5 bg-body rounded   " >
            ...
            <div className='py-2 px-4'>
                <h1 className='text-primary' >Service A</h1>
            ...
            <div>
                ...
                <MDBModal staticBackdrop tabIndex='-1' show={staticModal} setShow={setStaticModal}>
                    <MDBModalDialog>
                        <MDBModalContent>
                            <MDBModalHeader>
                                <MDBModalTitle> Service A </MDBModalTitle>
                            ...
                            <MDBModalFooter>
                                <div className={footerM.btnsA}>
                                    <MDBBtn color='secondary' className='me-2 px-4' onClick={()=>{ toggleShow(); window.location.reload() }}>
                                        Fermer
                                    </MDBBtn>
